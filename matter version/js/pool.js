  ...
  class Ball {
    ...
    shoot(xVel,yVel){
      ...
      Body.setAngle(this.body,Math.atan(yVel/xVel));
      const force = p5.Vector.fromAngle(this.body.angle)
      Body.applyForce(this.body,this.body.position,force * 0.002);
      // projection.on = false;
  ...
  class Projection{
    constructor(ball){
      this.x1 = ball.position.x
      this.y1 = ball.position.y
      this.x2 = ball.position.x
      this.y2 = ball.position.y
      this.on = true
    }
    updateX(xVel){
  ...
  function setup() {
    ...
    setupRackOfBalls();
    projection = new Projection(balls[0]);
    cue = new Cue(200,250,balls[0]);
    gameStarted = true;
  }
